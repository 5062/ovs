...
mf_set_wild(const struct mf_field *mf, struct match *match, char **err_str)
{
    ...
    case MFF_TCP_SEQ:
        match->wc.masks.tcp_seq = htonl(0);
        match->flow.tcp_seq = htonl(0);
        break;

    case MFF_TCP_ACK:
        match->wc.masks.tcp_ack = htonl(0);
        match->flow.tcp_ack = htonl(0);
        break;
       ...
       struct match *match, char **err_str)
{
    ...
    case MFF_TCP_SEQ:
        match_set_tcp_seq_masked(match, value->be32, mask->be32);
        break;
